...
class TestValidationService:
    ...
    @pytest.mark.asyncio
    async def test_validate_artifacts_success(self, validation_service, sample_artifacts):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(validation_service, '_run_command') as mock_run:
                ...
                result = await validation_service.validate_artifacts(temp_path, sample_artifacts)
    ...
    @pytest.mark.asyncio
    async def test_terraform_validation_success(self, validation_service, sample_artifacts):
        """Test successful Terraform validation."""
        terraform_artifacts = [a for a in sample_artifacts if a["kind"] == "infrastructure"]
  ...
  tags = {
    Name = "Example"
            ...
            with patch.object(validation_service, '_run_command') as mock_run:
                ...
                result = await validation_service._run_terraform_validation(temp_path, terraform_artifacts)
                
                assert result["status"] == "pass"
                assert len(result["issues"]) == 0
    
    @pytest.mark.asyncio
    async def test_terraform_validation_failure(self, validation_service, sample_artifacts):
    ...
    @pytest.mark.asyncio
    async def test_python_validation_success(self, validation_service, sample_artifacts):
...
if __name__ == "__main__":
    main()
            ...
            with patch.object(validation_service, '_run_command') as mock_run:
                ...
                result = await validation_service._run_python_validation(temp_path, python_artifacts)
                
                assert result["status"] == "pass"
                assert len(result["issues"]) == 0
    
    @pytest.mark.asyncio
    async def test_python_validation_lint_errors(self, validation_service, sample_artifacts):
    ...
    @pytest.mark.asyncio
    async def test_security_validation_secrets_found(self, validation_service, sample_artifacts):
...
AWS_SECRET_ACCESS_KEY = "wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY"
            ...
            result = await validation_service._run_security_validation(temp_path, sample_artifacts)
    ...
    @pytest.mark.asyncio
    async def test_security_validation_no_secrets(self, validation_service, sample_artifacts):
    ...
    @pytest.mark.asyncio
    async def test_general_validation_missing_files(self, validation_service, sample_artifacts):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result = await validation_service._run_general_validation(temp_path, sample_artifacts)
    ...
    async def test_general_validation_large_files(self, validation_service):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result = await validation_service._run_general_validation(temp_path, large_artifacts)
    ...
    def test_determine_overall_status(self, validation_service):
        ...
        assert validation_service._determine_overall_status(results_with_error) == "error"
        ...
        assert validation_service._determine_overall_status(results_with_warning) == "warning"
    ...
    async def _create_valid_test_files(self, temp_path: Path, artifacts):
        ...
        (temp_path / "README.md").write_text("# MLOps Project\n\nGenerated by Agentic MLOps Platform")
...
class TestValidationServiceIntegration:
    ...
    async def test_end_to_end_validation(self):
        ...
        artifacts = [
            {"path": "README.md", "kind": "documentation", "size_bytes": 100, "created_at": "2025-01-01T00:00:00Z"},
            {"path": "terraform/main.tf", "kind": "infrastructure", "size_bytes": 200, "created_at": "2025-01-01T00:00:00Z"},
            {"path": "src/app.py", "kind": "application", "size_bytes": 150, "created_at": "2025-01-01T00:00:00Z"}
        ]
  ...
  ami = "ami-12345678"
            ...
            with patch.object(validation_service, '_run_command') as mock_run:
                ...
                def mock_command(cmd, cwd):
                    if "terraform" in cmd:
                        if "init" in cmd:
                            return {"returncode": 0, "stdout": "Terraform initialized", "stderr": ""}
                ...
                result = await validation_service.validate_artifacts(temp_path, artifacts)
