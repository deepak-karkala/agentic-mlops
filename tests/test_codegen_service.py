...
class TestCodegenService:
    ...
    @pytest.mark.asyncio
    async def test_generate_mlops_repository_success_with_claude_sdk(
        self, codegen_service, sample_plan
    ):
        """Test successful MLOps repository generation with Claude SDK."""
        # Mock Claude Code SDK
        with (
            patch("libs.codegen_service.ClaudeCodeOptions") as mock_options_class,
            ...
            mock_messages = [
                {
                    "type": "file_created",
                    "file_info": {"path": "terraform/main.tf", "size": 1024},
                },
                {
                    "type": "file_created",
                    "file_info": {"path": "src/main.py", "size": 512},
                },
    ...
    @pytest.mark.asyncio
    async def test_generate_mlops_repository_fallback(
        self, codegen_service, sample_plan
    ):
    ...
    async def test_s3_upload_success(self, codegen_service):
        ...
        with tempfile.NamedTemporaryFile(suffix=".zip") as temp_file:
            ...
            assert (
                s3_url == "s3://test-bucket/artifacts/test_project_20250101_120000.zip"
            )
    ...
    async def test_repository_generation_error_handling(self, codegen_service):
        ...
        with patch("libs.codegen_service.ClaudeSDKClient") as mock_client_class:
            ...
            with patch.object(
                codegen_service, "_fallback_template_generation"
            ) as mock_fallback:
...
class TestCodegenServiceIntegration:
    ...
    async def test_end_to_end_repository_creation(self):
        ...
        sample_plan = {
            ...
            "key_services": {"ec2": "Compute instances", "rds": "Database"},
