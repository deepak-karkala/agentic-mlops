...
from libs.graph import _codegen_async, _validators_async, _create_validation_test_structure
...
class TestCodegenIntegration:
    ...
    async def test_codegen_with_approved_plan(self, sample_state_with_approved_plan):
        ...
        with patch('libs.codegen_service.CodegenService') as mock_service_class:
            ...
            mock_service.generate_mlops_repository = AsyncMock(return_value={
                "artifacts": [
                    {
    ...
    async def test_validators_with_artifacts(self):
        ...
        with patch('libs.validation_service.ValidationService') as mock_service_class:
            ...
            mock_service.validate_artifacts = AsyncMock(return_value={
                "terraform_validate": {"status": "pass", "issues": []},
                "ruff_check": {"status": "pass", "issues": []},
                "security_scan": {"status": "pass", "secrets_found": 0, "issues": []},
    ...
    async def test_validators_service_failure(self):
        ...
        state_with_artifacts = MLOpsWorkflowState(
            ...
            version=1,
            artifacts=[{"path": "test.py", "kind": "application", "size_bytes": 100, "created_at": "2025-01-01T00:00:00Z"}]
        )
    ...
    async def test_create_validation_test_structure(self):
        ...
        artifacts = [
            {"path": "terraform/main.tf", "kind": "infrastructure", "size_bytes": 100, "created_at": "2025-01-01T00:00:00Z"},
            {"path": "src/app.py", "kind": "application", "size_bytes": 200, "created_at": "2025-01-01T00:00:00Z"},
            {"path": ".github/workflows/ci.yml", "kind": "ci_cd", "size_bytes": 150, "created_at": "2025-01-01T00:00:00Z"}
        ]
    ...
    @pytest.mark.asyncio
    async def test_full_codegen_to_validation_workflow(self, sample_state_with_approved_plan):
        """Test complete workflow from code generation to validation."""
        # Mock both services
        with patch('libs.codegen_service.CodegenService') as mock_codegen_service_class, \
             patch('libs.validation_service.ValidationService') as mock_validation_service_class:
            
            # Setup codegen mock
            mock_codegen_service = Mock()
            mock_codegen_service_class.return_value = mock_codegen_service
            mock_codegen_service.generate_mlops_repository = AsyncMock(return_value={
                "artifacts": [
                    {"path": "terraform/main.tf", "kind": "infrastructure", "size_bytes": 1024, "created_at": "2025-01-01T00:00:00Z"},
                    {"path": "src/main.py", "kind": "application", "size_bytes": 512, "created_at": "2025-01-01T00:00:00Z"}
                ],
                "repository_zip": {
                    "local_path": "/tmp/repo.zip",
                    "s3_url": "s3://test-bucket/artifacts/test.zip",
            ...
            mock_validation_service.validate_artifacts = AsyncMock(return_value={
                "terraform_validate": {"status": "pass", "issues": []},
                "ruff_check": {"status": "pass", "issues": []},
                "security_scan": {"status": "pass", "secrets_found": 0, "issues": []},
                "general_checks": {"status": "pass", "issues": []},
