...
class ValidationService:
    ...
    async def validate_artifacts(
        self, artifacts_dir: Path, artifacts: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        ...
        validation_results = {
            "terraform_validate": await self._run_terraform_validation(
            ...
            "general_checks": await self._run_general_validation(
                artifacts_dir, artifacts
            ),
        ...
        logger.info(
            f"Validation completed with status: {validation_results['overall_status']}"
        )
        return validation_results

    async def _run_terraform_validation(
        self, artifacts_dir: Path, artifacts: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        ...
        if not terraform_files:
            return {
                "status": "skipped",
                "reason": "No Terraform files found",
                "issues": [],
            }
        ...
        try:
            ...
            if not terraform_dir.exists():
                return {
                    "status": "skipped",
                    "reason": "Terraform directory not found",
                    "issues": [],
                }
            ...
            format_result = await self._run_command(
                ["terraform", "fmt", "-check", "-diff"], str(terraform_dir)
            )
            ...
            init_result = await self._run_command(
                ["terraform", "init", "-backend=false"], str(terraform_dir)
            )
            ...
            validate_result = await self._run_command(
                ["terraform", "validate"], str(terraform_dir)
            )
    ...
    async def _run_python_validation(
        ...
        python_files = [
            a
            for a in artifacts
            if a.get("kind") == "application" and a.get("path", "").endswith(".py")
        ]
        ...
        try:
            ...
            check_result = await self._run_command(
                ["ruff", "check", "--output-format=json", "."], str(artifacts_dir)
            )
                ...
                try:
                    ...
                    for issue in ruff_issues:
                        ...
                        is_warning = (
                            ...
                            or code.startswith("N")  # pycodestyle naming
                            or code.startswith("D")
                        )  # pydocstyle
            ...
            format_result = await self._run_command(
                ["ruff", "format", "--check", "."], str(artifacts_dir)
            )
            if format_result["returncode"] != 0:
                issues.append(
                    {
    ...
    async def _run_security_validation(
        ...
        try:
            ...
            status = (
                "fail"
                if secrets_found > 0 or any(i["severity"] == "critical" for i in issues)
                else ("warning" if issues else "pass")
            )

            return {"status": status, "secrets_found": secrets_found, "issues": issues}
    ...
    async def _run_general_validation(
        self, artifacts_dir: Path, artifacts: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
    ...
    async def _generate_validation_report(
        self, artifacts_dir: Path, results: Dict[str, Any]
    ) -> None:
