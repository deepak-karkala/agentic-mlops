...
class ReasonCard(BaseModel):
    ...
    decision_set_id: str = Field(
        description="ID of the decision set this card belongs to"
    )

    # Timing
    timestamp: datetime = Field(default_factory=lambda: datetime.now(timezone.utc))
    duration_ms: Optional[int] = Field(
        None, description="Time taken for this reasoning step in milliseconds"
    )

    # Reasoning content
    reasoning: str = Field(
        description="Human-readable explanation of the agent's reasoning"
    )
    ...
    inputs: Optional[Dict[str, Any]] = Field(
        None, description="Key inputs that influenced this decision"
    )
    outputs: Optional[Dict[str, Any]] = Field(
        None, description="Structured outputs or results"
    )
    alternatives_considered: Optional[list[str]] = Field(
        None, description="Alternative options that were evaluated"
    )

    # Categorization
    category: str = Field(
        description="Category of reasoning (e.g., 'cost-analysis', 'technology-selection')"
    )
    priority: str = Field(
    ...
    references: Optional[list[str]] = Field(
        None, description="References to external resources or policies"
    )
...
class WorkflowProgress(BaseModel):
    ...
    def to_stream_event(self) -> StreamEvent:
        ...
        return StreamEvent(
            ...
            else StreamEventType.WORKFLOW_START,
            decision_set_id=self.decision_set_id,
            data={
...
class StreamWriter:
    ...
    def emit_workflow_paused(
        self, reason: str, node_name: Optional[str] = None
    ) -> None:
