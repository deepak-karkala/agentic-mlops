...
class CodegenService:
    ...
    async def generate_mlops_repository(self, plan: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        logger.info("Starting MLOps repository generation", extra={
    ...
    async def _generate_code_with_claude(self, plan: Dict[str, Any], output_dir: Path) -> List[Dict[str, Any]]:
                ...
                async for message in client.receive_response():
                    if message.get("type") == "file_created":
                        ...
                        artifacts.append({
                            ...
                            "kind": self._classify_file_kind(file_info.get("path", "")),
    ...
    async def _fallback_template_generation(self, plan: Dict[str, Any], output_dir: Path) -> List[Dict[str, Any]]:
...
Focus on creating production-quality code that follows MLOps best practices."""
    ...
    def _create_generation_prompt(self, plan: Dict[str, Any]) -> str:
        ...
        services_list = "\n".join([f"- {svc}: {desc}" for svc, desc in services.items()])
...
Generate all necessary files with proper content, following AWS and MLOps best practices. Make sure the code is production-ready and well-documented."""
    ...
    async def _create_repository_zip(self, source_dir: Path, zip_path: Path, plan: Dict[str, Any]) -> str:
